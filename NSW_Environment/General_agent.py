...
class RLAgent():
        ...
        -Atributes
                ...
                - type AC , DQN 
                - ID to which signal controler the agent is talking to.
        ...
        def save_agent(self,
                        ...
                        episode,
                        save_location = None):
                ''' Saves agent.weights, agent.memory, training results, Losses.

                # Args
                        -- vissim_working_directory
                        -- model_name - any name you like
                        -- agent_type - use 'AC' or else requires an agent.model.save method
                        -- Session_ID - any name you like eg. 'AC'
                        -- episode - number of episodes when saved
                '''
                # Chech if suitable folder exists
                folder =  os.path.join(vissim_working_directory,
                                       model_name,
                                       "Agents_Results",
                                       agent_type,
                                       Session_ID)
                ...
                if self.type == 'AC':
                        Weights_Filename = os.path.join(vissim_working_directory,
                                                        ...
                                                        agent_type,
                                                        Session_ID,
                                                        'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Weights'+'.h5')
                        ...
                        Optimizer_Filename = os.path.join(vissim_working_directory,
                                                          ...
                                                          agent_type,
                                                          Session_ID,
                                                          'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Optimizer'+'.h5')

                        if save_location is not None :
                                Weights_Filename = os.path.join(save_location,
                                                                'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Weights'+'.h5')
                                                                
                                Optimizer_Filename = os.path.join(save_location,
                                                                'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Optimizer'+'.h5')
                        
                        print('Saving architecture, weights, optimizer state for agent-{}'.format(self.ID))
                        print('Saving at : ',Weights_Filename)
                ...
                Memory_Filename = os.path.join(vissim_working_directory,
                                               model_name,
                                               "Agents_Results",
                                               agent_type,
                                               Session_ID,
                                               'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Memory'+'.p')
                
                print('Dumping agent-{} memory into pickle file'.format(self.ID))
                pickle.dump(self.memory, open(Memory_Filename, 'wb'))
                
                Training_Progress_Filename = os.path.join(vissim_working_directory,
                                                          model_name,
                                                          "Agents_Results",
                                                          agent_type,
                                                          Session_ID,
                                                          'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Train'+'.p')

                print('Dumping Training Results into pickle file.')
                pickle.dump(self.reward_storage, open(Training_Progress_Filename, 'wb'))
                Loss_Filename = os.path.join(vissim_working_directory,
                                             model_name,
                                             "Agents_Results",
                                             agent_type,
                                             Session_ID,
                                             'Episode'+ str(episode) +'Agent'+str(self.ID)+'_Loss'+'.p')
        ...
        def load_agent(self,
                ...
                if self.type == 'AC':
                        if best:
                                Weights_Filename \
                                = os.path.join(vissim_working_directory,
                                                model_name,
                                                "Agents_Results",
                                                agent_type,
                                                Session_ID,
                                ...
                                if load_location is not None :
                                        Weights_Filename = os.path.join(load_location,
                                                                        'BestAgent'+str(self.ID)+'_Weights'+'.h5')
                                                                        
                                        Optimizer_Filename = os.path.join(load_location,
                                                                        'BestAgent'+str(self.ID)+'_Optimizer'+'.h5')
                        ...
                        print('loaded from ',Weights_Filename)
        ...
        def load_isolated_agent(self, vissim_working_directory, model_name, agent_type, Session_ID, episode, best = True):
                ...
                # Junctions 1,2,4 and 11,12 train together, so they need same loading ID when loading fresh
                if self.ID == 1:
                        load_id = 0
                elif self.ID == 3:
                        load_id = 0
                ...
                if self.type == 'AC':
                        if best:
                                Weights_Filename = os.path.join(vissim_working_directory,
                                                                ...
                                                                agent_type,
                                                                Session_ID,
                                                                "Agent{}".format(self.ID),'BestAgent'+str(load_id)+'_Weights'+'.h5')
...
# Reload agents that have already worked together in Balance from Agents_results folder
        def load_integrated_agent(self, vissim_working_directory, model_name, agent_type, Session_ID, episode, best = True, Memory_Required = False):
