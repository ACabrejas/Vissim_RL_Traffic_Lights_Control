	...
	for index, agent in enumerate(Agents):
		...
		# Update the agent
			...
			if agent.intermediate_phase == False:
				...
				agent.reward   = calculate_reward(agent, state_type)
...
def calculate_reward(agent, state_type):
	if state_type == 'Queues':
    	...
    	#print(reward)
	if state_type == 'QueuesDifference':
...
def PER_prepopulate_memory(Agents, Vissim, state_type, state_size, memory_size, vissim_working_directory, model_name, Session_ID, seconds_per_green, seconds_per_yellow, timesteps_per_second, demand_list, demand_change_timesteps):
	...
	if not os.path.exists(PER_prepopulation_directory):
		os.makedirs(PER_prepopulation_directory)
	# Chech if suitable file exists
	PER_prepopulation_filename =  os.path.join(PER_prepopulation_directory, Session_ID	+ '_PER_Prepop_len_'+ str(memory_size) +'.p')
